...
vi.mock('../../api.js', () => ({
  // The server wraps both payloads in a `data` envelope; the resource must
  // unwrap them and read the real field names (trend.health, dimension.dimension).
    ...
    if (path === '/api/v1/analytics/summary') {
      return Promise.resolve({
        data: {
          ...
          avg_health_score: 74,
          trends: [
            { date: '2024-11-25', findings: 45, resolved: 4, health: 62 },
            { date: '2024-12-02', findings: 41, resolved: 6, health: 65 },
            { date: '2024-12-09', findings: 38, resolved: 5, health: 68 },
            { date: '2024-12-16', findings: 33, resolved: 7, health: 71 },
            { date: '2024-12-23', findings: 29, resolved: 4, health: 74 },
          ],
        },
      });
    }
    if (path === '/api/v1/health-score') {
      return Promise.resolve({
        data: {
          ...
          status: 'analyzed',
          dimensions: [
            { dimension: 'Architecture', score: 78, trend: 'improving' },
            { dimension: 'Security', score: 65, trend: 'improving' },
            { dimension: 'Testing', score: 82, trend: 'stable' },
            { dimension: 'Documentation', score: 55, trend: 'improving' },
            { dimension: 'Reliability', score: 70, trend: 'improving' },
            { dimension: 'Developer Experience', score: 73, trend: 'stable' },
          ],
        },
      });
    }
...
describe('registerAnalyticsResources', () => {
  ...
  describe('analytics-summary handler', () => {
    ...
    it('identifies documentation as lowest-scoring dimension', async () => {
      const handler = getHandler();
      const result = await handler({ href: 'recurrsive://analytics/summary' });
      expect(result.contents[0].text).toContain('Documentation is the lowest-scoring dimension');
    });
    ...
    it('contains analysis runs count', async () => {
      const handler = getHandler();
      const result = await handler({ href: 'recurrsive://analytics/summary' });
      expect(result.contents[0].text).toContain('Analysis Runs');
    });
