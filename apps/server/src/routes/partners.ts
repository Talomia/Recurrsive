...
export async function registerPartnerRoutes(app: FastifyInstance): Promise<void> {
   ...
   */
  app.get(prefix, { preHandler: [optionalAuth] }, async (request, reply) => {
   ...
   * Get partner detail.
   */
  app.get(`${prefix}/:id`, { preHandler: [optionalAuth] }, async (request, reply) => {
   ...
   */
  app.get(`${prefix}/certifications`, { preHandler: [optionalAuth] }, async (_request, reply) => {
    return reply.send({
