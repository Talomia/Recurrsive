 ...
 *
 * Uses in-memory mock data pre-populated with 5 experiments.
 *
...
// ---------------------------------------------------------------------------
// Pre-populated mock data — 5 experiments
// ---------------------------------------------------------------------------
...
const experiments: Experiment[] = [...MOCK_EXPERIMENTS];
...
export async function registerExperimentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/experiments', async (request, reply) => {
    ...
    if (status) {
      filtered = experiments.filter((e) => e.status === status);
    }
